   ...
   public class CcComponentLoader extends EventDispatcher implements IRegulatedProcess
   {
      ...
      private function onSwfLoaded(param1:Event) : void
      {
         ...
         try
         {
            ...
            this._stream = null;
            _loc2_ = this._key.split(".")[1];
            _loc3_ = Util.getFlashVar();
            if(_loc3_.getValueByKey(ServerConstants.PARAM_ISOFFLINE) != "1")
            {
               if(CcLibConstant.ALL_LIBRARY_TYPES.indexOf(_loc2_) == -1)
               {
                  (_loc4_ = new UtilCrypto()).decrypt(this._swfBytes);
                  _loc4_ = null;
               }
            }
            this.dispatchEvent(new Event(Event.COMPLETE));
