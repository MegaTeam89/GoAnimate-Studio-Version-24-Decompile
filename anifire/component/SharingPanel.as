   ...
   public class SharingPanel extends Canvas implements IBindingClient
   {
      ...
      public function SharingPanel()
      {
         ...
         this._documentDescriptor_ = new UIComponentDescriptor({
            "type":Canvas,
            "events":{
               "creationComplete":"___SharingPanel_Canvas1_creationComplete",
               "initialize":"___SharingPanel_Canvas1_initialize"
            },
      ...
      public function init() : void
      {
         ...
         if(this.isOnStudio == false)
         {
            ...
            if(this.movieId == "")
            {
               this.movieId = FlexGlobals.topLevelApplication.parameters["movieId"];
            }
      ...
      public function shareAnimation(param1:String, param2:Boolean = false) : void
      {
         ...
         var _loc3_:* = "";
         var _loc4_:String = escape(ServerConstants.MOVIE_PATH + this.movieId + "?utm_source=" + param1 + "&uid=" + this.userId);
         var _loc5_:String = escape(UtilLicense.getCurrentLicensorDisplayName() + ": " + this.movieTitle);
         if(param1 == TYPE_FACEBOOK)
         {
            _loc3_ = "http://www.facebook.com/sharer.php?u=" + _loc4_ + "&t=" + _loc5_;
         }
         else if(param1 == TYPE_MYSPACE)
         {
            _loc3_ = "http://www.myspace.com/Modules/PostTo/Pages/?t=" + _loc5_ + "&c=" + escape(this.embedTag) + "&u=" + _loc4_ + "&l=1";
         }
         else if(param1 == TYPE_DIGG)
         {
            _loc3_ = "http://digg.com/submit?phase=2&url=" + _loc4_ + "&title=" + _loc5_ + "&bodytext=" + escape(this.movieDesc) + "&topic=videos_animation";
         }
         else if(param1 == TYPE_DELICIOUS)
         {
            _loc3_ = "http://del.icio.us/post?noui&v=4&jump=close&url=" + _loc4_ + "&title=" + _loc5_ + "&partner=GoAnimate";
         }
         else if(param1 == TYPE_REDDIT)
         {
            _loc3_ = "http://reddit.com/submit?url=" + _loc4_ + "&title=" + _loc5_;
         }
         else if(param1 == TYPE_STUMBLEUPON)
         {
            _loc3_ = "http://www.stumbleupon.com/submit?url=" + _loc4_ + "&title=" + _loc5_;
         }
         else if(param1 == TYPE_FARK)
         {
            _loc3_ = "http://cgi.fark.com/cgi/fark/submit.pl?new_url=" + _loc4_ + "&new_comment=" + escape(this.movieDesc);
         }
         if(this._isGo)
         {
            try
            {
               Util.gaTracking("/share/" + param1 + "/flash",this.stage);
            }
      ...
      private function createAnimation(param1:Boolean = false, param2:Boolean = false) : void
      {
         ...
         else
         {
            ...
            if(param1)
            {
               _loc6_ = this._movieId;
               _loc3_ += "copy/" + _loc6_;
            }
            _loc5_ = true;
         }
         if(this._isLogin || _loc5_)
         {
            _loc7_ = new URLRequest(_loc3_);
            navigateToURL(_loc7_,"_blank");
         }
         else
         {
      ...
      private function sendEmail() : void
      {
         ...
         else
         {
            ...
            _loc1_[ServerConstants.PARAM_MOVIE_ID] = this._movieId;
            _loc1_[ServerConstants.PARAM_SENDER_NAME] = this._txtSenderName.text;
            _loc1_[ServerConstants.PARAM_SENDER_EMAIL] = this._txtSenderEmail.text;
            _loc1_[ServerConstants.PARAM_RECIPIENT_EMAIL] = this._txtRecipients.text;
            _loc1_["custom_message"] = this._txtMessage.text;
            if(UtilLicense.isBoxEnvironment())
            {
               _loc1_[ServerConstants.PARAM_BOX_PARENT_URL] = Util.getFlashVar().getValueByKey(ServerConstants.PARAM_BOX_PARENT_URL);
               _loc1_[ServerConstants.PARAM_BOX_PHP_SESSION_ID] = Util.getFlashVar().getValueByKey(ServerConstants.PARAM_FLASHVAR_PHP_SESSION_ID);
            }
            _loc2_ = new URLRequest(ServerConstants.ACTION_SEND_SHARE_EMAIL);
            _loc2_.method = URLRequestMethod.POST;
            _loc2_.data = _loc1_;
            _loc3_ = new URLStream();
            _loc3_.addEventListener(Event.COMPLETE,this.sendEmailComplete);
            _loc3_.load(_loc2_);
