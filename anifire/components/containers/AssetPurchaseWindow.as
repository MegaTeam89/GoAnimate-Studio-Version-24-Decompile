   ...
   public class AssetPurchaseWindow extends TitleWindow implements IBindingClient
   {
      ...
      public function AssetPurchaseWindow()
      {
         ...
         this._documentDescriptor_ = new UIComponentDescriptor({
            "type":TitleWindow,
            "events":{"creationComplete":"___AssetPurchaseWindow_TitleWindow1_creationComplete"},
         ...
         {
            return AssetPurchaseWindow[param1];
         },bindings,watchers);
      ...
      public function set thumb(param1:Thumb) : void
      {
         ...
         else if(thumb.imageData)
         {
            if(thumb is CharThumb && (thumb as CharThumb).isCC)
            {
               ...
               ccMaker.initBySwfs(args["xml"] as XML,args["imageData"] as UtilHashBytes);
            }
            else
            {
               ba = new ByteArray();
               ...
               {
                  ba.writeBytes(ByteArray(thumb.imageData["figure"]));
               }
               loader = new Loader();
               this._dobj = new Image();
      ...
      private function purchase() : void
      {
         ...
         if(this._okPurchase)
         {
            ...
            else
            {
               ...
               _loc4_["uid"] = FlexGlobals.topLevelApplication.parameters["userId"];
               _loc4_["aid"] = this.aid;
      ...
      private function purchaseResult(param1:Event) : void
      {
         ...
         if(result.charAt(0) == "0")
         {
            ...
            UtilUser.goBuck -= Number(this._thumb.cost[0]);
            UtilUser.goPoint -= Number(this._thumb.cost[1]);
            if(this.thumb is CharThumb && (this.thumb as CharThumb).isCC)
            {
               ...
               try
               {
                  trackModel = JS0N.decode(i.track.toString())["parts"];
               }
               catch(e2:Error)
               {
               }
               js = StringUtil.substitute("CCStandaloneBannerAdUI.gaLogTx.logCCPartsNormal({0}, {1}, {2})",new_asset_id,JS0N.encode(trackModel.filter(function(param1:*, param2:int, param3:Array):Boolean
               {
