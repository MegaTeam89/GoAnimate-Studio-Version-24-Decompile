package anifire.components.studio
{
   ...
   public class MicRecordingComponent extends Canvas implements IBindingClient
   {
      ...
      public function init(param1:SpeechComponent = null) : void
      {
         ...
         {
            UtilPopUp.errorAlert("Error: MicRecordingPanel fail.");
            throw e;
         }
      }
      ...
      {
         this._helper = MicRecordingHelper.getInstance();
      ...
      private function oggEncodeComplete(param1:Event) : void
      {
         ...
         this.stream.addEventListener(Event.COMPLETE,this._bottom.onSentCompleteHandler);
         this.stream.addEventListener(Event.COMPLETE,this.onUploadComplete);
         this.stream.addEventListener(UtilURLStream.TIME_OUT,this.error);
         this.stream.addEventListener(IOErrorEvent.IO_ERROR,this.error);
         this.stream.addEventListener(SecurityErrorEvent.SECURITY_ERROR,this.error);
      ...
      private function onSoundAdded(param1:SoundEvent) : void
      {
         ...
         (_loc6_ = new AddSpeechCommand(Console.getConsole().speechManager.micRecordingManager.sounds.getValueByKey(_loc4_) as AnimeSound,_loc5_)).execute();
         if(this.target is Character)
         {
            _loc11_ = false;
      ...
      private function _MicRecordingComponent_bindingsSetup() : Array
      {
         ...
         },null,"_MicRecordingComponent_Label1.text");
         return result;
      }
