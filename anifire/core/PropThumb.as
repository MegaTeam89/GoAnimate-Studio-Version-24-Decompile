package anifire.core
{
   ...
   public class PropThumb extends Thumb implements IPropProduct
   {
      ...
      override public function deSerializeFacial(param1:XML, param2:ITheme, param3:int = -1, param4:int = 1) : void
      {
         ...
         setFileName("char/" + param1.@id + "/head/" + _loc5_.@id);
         thumbId = _loc5_.@id;
         id = param1.@id + ".head";
         aid = param1.@aid;
         name = param1.@name + "\'s head";
         premium = _loc5_.@is_premium == "Y" ? true : false;
         cost = [_loc5_.@money,_loc5_.@sharing];
         ...
         while(_loc11_ < _loc9_.length())
         {
            ...
            if(_loc8_.isEnable)
            {
               Console.getConsole().addStoreCollection(_loc8_.name);
               this.addStateMenuItem(_loc8_);
            }
            _loc11_++;
         }
      }
         ...
         if(param1.theme.id == this.theme.id && param1.id == this.id)
         {
            ...
            while(_loc4_ < this.states.length)
            {
               ...
               if(_loc2_.imageData == null)
               {
                  ...
                  {
                     _loc3_ = PropThumb(param1).states[_loc6_] as State;
            ...
            {
               _loc3_ = PropThumb(param1).states[_loc5_] as State;
      ...
      override public function doDrag(param1:MouseEvent) : void
      {
         ...
         if(purchased && this.imageData)
         {
            ...
            _loc4_ = new Image();
            (_loc5_ = new BitmapData(AnimeConstants.TILE_PROP_WIDTH,AnimeConstants.TILE_PROP_HEIGHT)).draw(ThumbnailCanvas(_loc2_.parent).displayObj);
      ...
      override public function loadImageData() : void
      {
         ...
         if(this.thumbId != null)
         {
            if(this.getFileName().indexOf("char/") == -1)
            {
               _loc1_ = UtilNetwork.getGetThemeAssetRequest(this.theme.id,this.id,ServerConstants.PARAM_PROP_STATE,this.thumbId,-1,path);
            }
            else
            {
               _loc1_ = UtilNetwork.getGetThemeAssetRequest(this.theme.id,this.id,ServerConstants.PARAM_CHAR_FACIAL,this.thumbId,-1,path);
            }
         }
         else
         {
