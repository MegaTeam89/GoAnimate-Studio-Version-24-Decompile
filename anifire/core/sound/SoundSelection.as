package anifire.core.sound
{
   ...
   public class SoundSelection extends EventDispatcher implements ICollection
   {
      ...
      public function removeSound(param1:AnimeSound) : Boolean
      {
         var _loc2_:int = 0;
         if(param1 && this._sounds.length > 1)
         {
            _loc2_ = this._sounds.getItemIndex(param1);
            if(_loc2_ >= 0)
            {
               this._sounds.removeItemAt(_loc2_);
               this.traceSound();
               this.dispatchEvent(new Event(Event.CHANGE));
               return true;
            }
