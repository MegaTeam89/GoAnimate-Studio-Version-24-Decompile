   ...
   public class SoundWaveformData extends EventDispatcher
   {
      ...
      public function init(param1:Sound) : void
      {
         ...
         if(!this._samples && param1)
         {
            ...
            _loc5_ = AnimeConstants.PIXEL_PER_SEC / PIXEL_PER_SAMPLE;
            _loc6_ = uint(_loc2_ / _loc5_) * 8;
            _loc7_ = 0;
            _loc8_ = 0;
            _loc9_ = 0;
            _loc4_.position = 0;
            while(_loc4_.bytesAvailable > 8)
            {
               ...
               _loc8_ = Math.max(_loc8_,Math.abs(_loc9_));
               this._samples.push(_loc9_);
               if(_loc4_.bytesAvailable <= _loc6_)
               {
                  break;
               }
            ...
            _loc10_ = 1000;
            _loc11_ = 0;
            while(_loc11_ < this._samples.length)
            {
               this._samples[_loc11_] = Math.round(_loc10_ * Math.abs(this._samples[_loc11_]) / _loc8_) / _loc10_;
               _loc11_++;
            }
         }
      }
      ...
      private function onUrlLoadComplete(param1:Event) : void
      {
         ...
         IEventDispatcher(param1.target).addEventListener(param1.type,this.onUrlLoadComplete);
         try
         {
