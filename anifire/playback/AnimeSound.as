package anifire.playback
{
   ...
   public class AnimeSound extends EventDispatcher
   {
      ...
      public static function createAndInitSounds(param1:XML, param2:UtilHashArray, param3:String, param4:PlayerDataStock) : Array
      {
         ...
         {
            var _loc7_:int = 0;
            var _loc8_:* = §§checkfilter(themeXml.sound);
            var _loc6_:* = new XMLList("");
            §§push(§§findproperty(XMLList));
            themeXml.sound.(@id == thumbId);
            §§pop().§§slot[13] = (§§newactivation()).XMLList(_loc6_)[0];
         }
      ...
      public function unmute_inner_volume() : void
      {
         this._inner_volume = this._inner_vol_bak;
      }
      ...
      public function init(param1:XML, param2:UtilHashArray, param3:String, param4:PlayerDataStock) : Boolean
      {
         ...
         if(soundXML.hasOwnProperty("@track"))
         {
            this._track = Number(soundXML.@track);
         }
         if(soundXML.hasOwnProperty("@tts") && String(soundXML.@tts) == "1")
         {
            this._isSpeech = true;
            this.setEndFrame(getEndFrameFromSoundXML(soundXML) + 48);
            this.updateEndMilliSecFromEndFrame();
         }
         ...
         try
         {
            ...
            §§push(§§findproperty(XMLList));
            themeXml.sound.(@id == thumbId);
            §§pop().§§slot[8] = (§§newactivation()).XMLList(_loc6_)[0];
